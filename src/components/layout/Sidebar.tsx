...
export const Sidebar: React.FC<SidebarProps> = ({ topics, publications, selectedEvent }) => {
  const filteredTopics = selectedEvent 
    ? topics.filter(topic => topic.eventId === selectedEvent.id)
    : topics
  ...
  return (
    <div className="w-80 h-full bg-[#4a2c2a] border-l border-[#eacaae] flex flex-col overflow-hidden">
      <div className="p-4 border-b border-[#eacaae]">
        ...
        <Tabs defaultValue="photos" className="h-full">
          ...
          <TabsContent value="photos" className="p-4 h-full">
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-[#f5f5f0] mb-3">Event Photos</h3>
              <div className="grid grid-cols-2 gap-3">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  ...
                  >
                    <span className="text-[#4a2c2a] text-xs">Photo {i}</span>
                  </div>
                ))}
              </div>
              <div className="mt-4 p-3 bg-[#8b4513] rounded-lg">
                <p className="text-xs text-[#f5f5f0]">
                  Photo placeholders for event locations and coffee moments. Upload feature coming soon.
                </p>
              </div>
            </div>
          </TabsContent>
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {filteredTopics.length === 0 ? (
                  ...
                  filteredTopics.map((topic) => (
                    <div
                      key={topic.id}
                      className="p-3 bg-[#8b4513] rounded-lg border border-[#e8ddd4]"
                    >
                      <h4 className="font-medium text-[#f5f5f0] text-sm mb-1">
                        {topic.theme}
                      </h4>
                      <p className="text-xs text-[#e8ddd4]">
                        {topic.description}
                      </p>
                    </div>
                  ))
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-[#e8ddd4] mb-2">
                            Type: {pub.type}
                          </p>
                        </div>
                      </div>
